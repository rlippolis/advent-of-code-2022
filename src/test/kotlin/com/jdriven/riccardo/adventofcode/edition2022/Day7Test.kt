package com.jdriven.riccardo.adventofcode.edition2022

import com.jdriven.riccardo.adventofcode.Utils
import org.junit.jupiter.api.Assertions.assertEquals
import org.junit.jupiter.api.Test

internal class Day7Test {

    private val example = Utils.readFile("edition2022/day7-example")
    private val input = Utils.readFile("edition2022/day7-input")

    @Test
    fun shouldSolveExamplePart1() {
        val result = Day7().solvePart1(example)
        assertEquals(95437, result)
    }

    @Test
    fun shouldSolveInputPart1() {
        val result = Day7().solvePart1(input)
        println(result)
        assertEquals(1444896, result)
    }

    @Test
    fun shouldSolveExamplePart2() {
        val result = Day7().solvePart2(example)
        assertEquals(24933642, result)
    }

    @Test
    fun shouldSolveInputPart2() {
        val result = Day7().solvePart2(input)
        println(result)
        assertEquals(404395, result)
    }
}
