package com.jdriven.riccardo.adventofcode.edition2022

import com.jdriven.riccardo.adventofcode.Utils
import org.junit.jupiter.api.Assertions.assertEquals
import org.junit.jupiter.api.Test

internal class Day1Test {

    private val example = Utils.readFile("edition2022/day1-example")
    private val input = Utils.readFile("edition2022/day1-input")

    @Test
    fun shouldSolveExamplePart1() {
        val result = Day1().solvePart1(example)
        assertEquals(24000, result)
    }

    @Test
    fun shouldSolveInputPart1() {
        val result = Day1().solvePart1(input)
        println(result)
        assertEquals(70369, result)
    }

    @Test
    fun shouldSolveExamplePart2() {
        val result = Day1().solvePart2(example)
        assertEquals(45000, result)
    }

    @Test
    fun shouldSolveInputPart2() {
        val result = Day1().solvePart2(input)
        println(result)
        assertEquals(203002, result)
    }
}
