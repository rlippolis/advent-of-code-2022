...
object Dijkstra {
    fun <T> findShortestRoute(nodes: Collection<T>, start: T, end: T, neighbourGetter: (T) -> Collection<Pair<T, Int>>): List<T>? {
        val nodeMapping = mutableMapOf<T, ShortestPathNode<T>>()
        nodes.forEach { node ->
            val pathNode = nodeMapping.getOrCreate(node)
    ...
    private fun <T> MutableMap<T, ShortestPathNode<T>>.getOrCreate(point: T, weight: Int = 0) =
